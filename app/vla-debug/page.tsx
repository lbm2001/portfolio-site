...
import { setRunConfig, type RunConfig } from "mini-vla/config";
import type { ProbeResponse } from "./probe.worker";

const LOG_KEY = "vla-debug-log";
const DONE = "— run finished cleanly —";

/** What Hero installs below 1100px. Identical compute to desktop: it only
    narrows the palette (numColors) and the scene size (maxBlocks). */
...
export default function VlaDebug() {
  ...
  /** One probe stage in the worker, resolved on its done/error reply. */
    ...
    (stage: "caps" | "tf" | "bench") =>
      new Promise<void>((resolve) => {
        ...
        const onMsg = (e: MessageEvent<ProbeResponse>) => {
          const m = e.data;
          if (m.t === "log") return append(m.line);
  ...
  const runWorkerTrainer = () =>
    guard(async () => {
      append("→ real VLATrainer (worker path, exactly what Hero uses)");
      const t = (trainerRef.current ??= new VLATrainer());
      t.start(watch("worker-trainer", () => t), MOBILE_RUN_CONFIG);
    });
  ...
  const runInlineTrainer = () =>
    guard(async () => {
      append("→ VLATrainerCore inline (main thread, no worker, no OffscreenCanvas GL)");
      setRunConfig(MOBILE_RUN_CONFIG);
      const c = (coreRef.current ??= new VLATrainerCore());
      void c.start(watch("inline-core", () => c));
    });
  ...
  return (
    <main style={{ padding: 16, maxWidth: 720, margin: "0 auto" }}>
      ...
      <button style={btn} disabled={busy} onClick={runWorkerTrainer}>
        4 · Real trainer — worker path
      </button>
      <button style={btn} disabled={busy} onClick={runInlineTrainer}>
        5 · Real trainer — main thread
      </button>
